...
def CreateBernoulli (device, region):
    ...
    EnsureEdgeFromNodeModelExists(device, region, "Potential")
    vdiffstr="(Potential@n0 - Potential@n1)/edgeV_t_T"
    CreateEdgeModel(device, region, "vdiff", vdiffstr)
    CreateEdgeModel(device, region, "vdiff:Potential@n0",  "edgeV_t_T^(-1)")
    ...
    CreateEdgeModelDerivatives(device, region,"vdiff",vdiffstr,"Temperature")
    CreateEdgeModelDerivatives(device, region,"Bern01","B(vdiff)","Temperature")
