...
def CreateJouleHeatSilicon(device,region):
    # WARNING (Phase4 审查标注): 下面 J.E 形式 "ElectricField*(ElectronCurrent+HoleCurrent)"
    # 在扩散主导区会给出负的 Joule 热(载流子逆电场扩散)，导致净热源为负、温度场无法升温。
    # 本函数当前为死代码：phase4 的 diode_1d_electrothermal.py / bjt_electrothermal.py 均不调用它，
    # 而是内联严格正定形式 Jn^2/(q*mu_nT*n) + Jp^2/(q*mu_pT*p)（与 CreateJouleHeatGaN 一致）。
    # 如需在硅器件上启用 Joule 热，请改用正定形式，勿直接使用本函数。
    ...
    JouleHeat ="ElectricField * (ElectronCurrent + HoleCurrent)"
...
def CreateJouleHeatGaN(device,region):
    edge_average_model(device=device, region=region, edge_model="edgeElectrons", node_model="Electrons")
    edge_average_model(device=device, region=region, edge_model="edgeHoles", node_model="Holes")
    JouleHeat ="ElectronCurrent*ElectronCurrent/(mu_vsat_e*ElectronCharge*edgeElectrons)+ HoleCurrent*HoleCurrent/(mu_p_Albrecht*ElectronCharge*edgeHoles)"
    #Heatsource ="ElectronCurrent*ElectronCurrent/(mu_nT*ElectronCharge*edgeElectrons)+ HoleCurrent*HoleCurrent/(mu_pT*ElectronCharge*edgeHoles)"
...
def CreateJouleHeatAlGaN(device,region):
    edge_average_model(device=device, region=region, edge_model="edgeElectrons", node_model="Electrons")
    edge_average_model(device=device, region=region, edge_model="edgeHoles", node_model="Holes")
    JouleHeat ="ElectronCurrent*ElectronCurrent/(mu_n_Albrecht*ElectronCharge*edgeElectrons)+ HoleCurrent*HoleCurrent/(mu_p_Albrecht*ElectronCharge*edgeHoles)"
    #Heatsource ="ElectronCurrent*ElectronCurrent/(mu_nT*ElectronCharge*edgeElectrons)+ HoleCurrent*HoleCurrent/(mu_pT*ElectronCharge*edgeHoles)"
    # Heatsource ="ElectricField * (ElectronCurrent + HoleCurrent)"
    CreateEdgeModel(device, region, "JouleHeat", JouleHeat)
    for v in ("Potential", "Electrons", "Holes", "Temperature"):
